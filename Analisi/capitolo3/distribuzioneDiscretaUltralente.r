...
glicemie_ultralente <- data %>%
  filter(UsesUltraLente == TRUE, Code >= 48 & Code <= 64)
...
pdf("./grafici_plot/ecdf_glicemia_pazienti_ultralente.pdf", width = 7, height = 5)

ggplot(medie_ultralente, aes(x = MediaGlicemia)) +
  stat_ecdf(geom = "step", color = "darkgreen", size = 1) +
  labs(
    title = "ECDF - Media glicemica per pazienti con insulina ultralente",
